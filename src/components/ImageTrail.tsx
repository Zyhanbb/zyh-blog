...
export default function ImageTrail({
  ...
  useEffect(() => {
    if (!enableClick || !containerRef.current) return;
    const handleClick = (e: MouseEvent) => {
      if (instanceRef.current && typeof instanceRef.current.showNextImage === 'function') {
        // Set mouse position to click position
        const rect = containerRef.current!.getBoundingClientRect();
        instanceRef.current.mousePos = {
          x: e.clientX - rect.left,
          y: e.clientY - rect.top,
        };
        instanceRef.current.cacheMousePos = { ...instanceRef.current.mousePos };
        instanceRef.current.showNextImage();
      }
    };
